...
void main() {
  testWidgets('shows two compact featured apps and opens an app detail route', (
    tester,
  ) async {
    await _pumpStorefront(tester, size: const Size(390, 1000));

    expect(find.byKey(const Key('production-apps-storefront')), findsOneWidget);
    expect(
      find.byKey(const Key('app-store-tile-van-cranenbroek')),
      findsOneWidget,
    );
    expect(
      find.byKey(const Key('app-store-tile-lyzer-collect-deliver')),
      findsOneWidget,
    );
    expect(
  ...
  testWidgets('opens the complete catalog and reaches the third app', (
    tester,
  ) async {
    await _pumpStorefront(tester, size: const Size(390, 1000));
